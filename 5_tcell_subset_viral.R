...
for (x in list("integrated.cca", "integrated.harmony", "integrated.joint", "integrated.rcpa")) {
    seu.obj <- dataVisUMAP(seu.obj = seu.obj, outDir = "../output/s3/", outName = paste0(outName, "_", x), 
                           final.dims = 30, final.res = 0.6, stashID = "clusterID_sub", algorithm = 3, min.dist = 0.3, n.neighbors = 30,
                           prefix = "RNA_snn_res.", assay = "RNA", reduction = x,
                           saveRDS = F, return_obj = T, returnFeats = T,
                           features = c("MKI67", "CD3E", "CD8A", "GZMA", 
                                        "CD163", "IL23R", "KLRB1", "CD7", "IFIT3", "HLA-DRA", 
                                        "CD4", "BLK", "EPCAM", "IL7R", "DNAJB1","CTLA4", "IL18", "CCR4", "GATA3")
                          )
}

saveRDS(seu.obj, paste0("../output/s3/", outName,"_S3.rds"))

#remove meyloid doublets (c10), and cycling T cells (c8)
seu.obj <- subset(seu.obj, invert = T, subset = clusterID_sub_integrated.harmony %in% c(1,6,9,10))
...
#remove low quality (TAP, CKB, RNASE6, EXT1, NFATC2, RBP4, NME8) (c6,7) ----I have not figured out how to make this work. place holder
seu.obj <- subset(seu.obj, invert = T, subset = clusterID_integrated.harmony %in% c(6,7))
...
for (x in list("integrated.cca", "integrated.harmony", "integrated.joint", "integrated.rcpa")) {
    seu.obj <- dataVisUMAP(seu.obj = seu.obj, outDir = "../output/s3/", outName = paste0(outName, "_", x), 
                           final.dims = 30, final.res = 0.3, stashID = "clusterID_sub", algorithm = 3, min.dist = 0.3, n.neighbors = 30,
                           prefix = "RNA_snn_res.", assay = "RNA", reduction = x,
                           saveRDS = F, return_obj = T, returnFeats = T,
                           features = c("CD3E", "CD8A", "CD4", "IL23R", 
                                        "BLK", "GZMA", "NCR1", "KLRB1", 
                                        "CD7", "DNAJB1", "IFIT3","CTLA4", "RBP4", "CKB", "RNASE6", "PTPRC")
                          )
}
saveRDS(seu.obj, paste0("../output/s3/", outName,"CLEANish_S3.rds"))
...
contrast <- c("Diseased", "Healthy") #code will test first vs second
seu.obj <- readRDS(paste0("../output/s3/", outName,"CLEANish_S3.rds"))
seu.obj <- loadMeta(seu.obj = seu.obj, metaFile = "./metaData/tcell_viral_ID.csv", groupBy = "clusterID_sub_integrated.harmony", metaAdd = "majorID_sub")
seu.obj <- loadMeta(seu.obj = seu.obj, metaFile = "./metaData/tcell_CLEANish_viral_ID.csv", groupBy = "clusterID_sub_integrated.harmony", metaAdd = "majorID_sub")
...
###originally from Dylan's "load in data and metadata code", but mine wouldn't run with it in, so I took out "orig" part of "majorID_sub_orig".
seu.obj <- loadMeta(seu.obj = seu.obj, metaFile = "./metaData/tcell_viral_ID.csv", groupBy = "clusterID_integrated.harmony", metaAdd = "majorID_sub_orig")

seu.obj <- convertTOclusID(seu.obj = seu.obj, metaSlot = "majorID_sub", newMetaName = "majorID_sub_num")
...
ExportToCB_cus(seu.obj = seu.obj, dataset.name = paste0(outName, "_CLEAN"), outDir = "../output/cb_input/", 
                markers = paste0("../output/viln/", outName, "/tcell_viral_majorID_sub_gene_list.csv"),
                reduction = reduction,  
...
### Generate dot plots using vilnPlots resuts of majorID NOTE: I changed "clusMain" to "majorID_sub", because I wasn't sure how to make the integrated harmony gene list. Can do by clusMain or by "majorID_sub" to label clusters with identities: But have to do something about the circles if you do majorID_sub
pi <- autoDot(
    seu.integrated.obj = seu.obj, inFile = paste0("../output/viln/", outName, "/", outName, "_", "majorID_sub", "_gene_list.csv"), 
    groupBy = clusMain, MIN_LOGFOLD_CHANGE = 0.5, MIN_PCT_CELLS_EXPR_GENE = 0.1, filterTerm = "ENSBTAG"
) + coord_flip()
ggsave(paste0("../output/", outName, "/", outName, "_autodot_clusID.png"), width = 8, height = 12)

pi <- autoDot(
    seu.integrated.obj = seu.obj, inFile = paste0("../output/viln/", outName, "/", outName, "_", "majorID_sub", "_gene_list.csv"), 
    groupBy = "majorID_sub", MIN_LOGFOLD_CHANGE = 0.5, MIN_PCT_CELLS_EXPR_GENE = 0.1, filterTerm = "ENSBTAG"
) + coord_flip()
ggsave(paste0("../output/", outName, "/", outName, "_autodot_majorID_sub_clusID.png"), width = 8, height = 12)

### Plot raw cluster UMAP
pi <- DimPlot(seu.obj, 
                reduction = reduction, 
                group.by = clusMain,
                pt.size = 0.25,
...
p <- cusLabels(plot = pi, shape = 21, size = 8, alpha = 0.8, labCol = "black", smallAxes = T) 
...
p <- cusLabels(plot = pi, shape = 21, size = 8, alpha = 0.8, labCol = "black", smallAxes = T) 
ggsave(paste0("../output/", outName, "/", outName, "_initUMAP.png"), width = 7, height = 7)



### Plot major cluster UMAP (numeric)
pi <- DimPlot(seu.obj, 
                reduction = reduction, 
                group.by = "majorID_sub_num",
...
p <- cusLabels(plot = pi, shape = 21, size = 8, alpha = 0.8, labCol = "black", smallAxes = T) 
ggsave(paste0("../output/", outName, "/", outName, "_majorNumUMAP.png"), width = 7, height = 7)


### Plot labeled major cluster UMAP
pi <- DimPlot(seu.obj, 
                reduction = reduction, 
...
### Key feature plots
features <- c("CD4", "KLRB1", "CD7", 
              "CD8A", "CCL5", "GZMK", "GZMA", 
              "CTLA4", "IL23R", "CTSW", "IFNG", 
              "BAG3", "DNAJB1", "BLK", "ISG15")
p <- prettyFeats(seu.obj = seu.obj, nrow = 3, ncol = 5, title.size = 14, features = features, order = F, legJust = "top", 
                 reduction = reduction) 
ggsave(paste0("../output/", outName, "/", outName, "_featPlots.png"), width = 15, height = 9)


### UMAP by sample -- if unequal sample size downsample by cellSource
seu.obj$name <- factor(seu.obj$name, levels = c("H_1", "H_2", "H_3", "H_4", "H_5", "Dz_1", "Dz_3", "Dz_5"))
seu.obj$colz <- factor(seu.obj$colz, levels = c("#004616", "#2B8238", "#6DB667", "#93CA8B", "#B4DCAB", "#645A9F", "#AF90D2", "#E4C3F2"))
...
### Frequency plots to run stats - cluster ASK DYLAN HOW TO CHANGE COLORS
...
# Run scProportion::permutation_test
log2FD_threshold <- 1
seu.obj$name <- droplevels(seu.obj$name)
Idents(seu.obj) <- "name"
set.seed(12)
...
geom_hline(yintercept = 0) + scale_color_manual(values = c("blue", "red","grey")
                                                        ...
                                                        ) + ylab(expression(atop(bold("abundance log2(FC)"),atop(italic("(BRD versus Healthy)"))))) + guides(color = guide_legend(nrow=1))
...
# define the cluster
lapply(c("T_HSP", "gd_T1", "gd_T1"), function(x){
    p_volc <- btwnClusDEG(
...
### For viral infection prediction, check out https://www.biorxiv.org/content/10.1101/2020.04.15.042978v1.full.pdf