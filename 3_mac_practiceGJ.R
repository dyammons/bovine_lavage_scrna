...
seu.obj <- subset(seu.obj, subset = majorID == "mac") #specify subset of interest
...
#rsync -avzP rsync://ftp.ensembl.org/ensembl/pub/release-112/fasta/bos_taurus/dna/Bos_taurus.ARS-UCD1.3.dna.toplevel.fa.gz .

cellranger mkgtf gtf_files.gtf gtf_files_filtered.gtf \
...
https://ftp.ensembl.org/ensembl/pub/release-112/gtf/bos_taurus/Bos_taurus.ARS-UCD1.3.112.gtf.gz
...
ggsave(paste0("../output/", outName, "/", outName, "_rawUMAPgj621.png"), width = 7, height = 7)
...
# With Macs identified as upregualted in disease, attempt to better
# define the cluster

lapply(c("13", "9", "2"), function(x){
    p_volc <- btwnClusDEG(
        ...
        minCells = 5, outDir = paste0("../output/", outName, "/"), title = paste0(x, "_VS_otherMacs"), 
        idents.1_NAME = x, idents.2_NAME = "Other Macs", returnVolc = T, 
        doLinDEG = F, paired = T, addLabs = NULL, lowFilter = T, dwnSam = F, setSeed = 24
       )

    p <- prettyVolc(
        plot = p_volc[[1]], rightLab = x, lfcCut = 1,
        leftLab = "Other Macs", arrowz = T
        ) + 
        labs(x = "log2(FC)") + 
        theme(
        ...
        ) + ggtitle(paste0(x, " vs other Macs"))
