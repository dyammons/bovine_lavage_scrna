...
#remove low quality T cells (c1 and c3), meyloid doublets (c7), and cycling T cells (c10)
seu.obj <- subset(seu.obj, invert = T, subset = clusterID_integrated.harmony %in% c(1, 3, 7, 10))
...
seu.obj <- loadMeta(seu.obj = seu.obj, metaFile = "./metaData/tcell_ID.csv", groupBy = "clusterID_integrated.harmony", metaAdd = "majorID_sub_orig")
seu.obj <- loadMeta(seu.obj = seu.obj, metaFile = "./metaData/tcell_CLEAN_ID.csv", groupBy = "clusterID_sub_integrated.harmony", metaAdd = "majorID_sub")
...
ggsave(paste0("../output/", outName, "/", outName, "_autodot_major_clusID.png"), width = 12, height = 4)
...
### Key feature plots
features <- c("CD4", "S100A13", "S100A11", 
              "CD8A", "CCL5", "GZMK", "GZMA", 
...
geom_hline(yintercept = 0) + scale_color_manual(values = c("blue", "red","grey")
                                                        ...
                                                        ) + ylab(expression(atop(bold("abundance log2(FC)"),atop(italic("(Diseased versus healthy)"))))) + guides(color = guide_legend(nrow=1))
